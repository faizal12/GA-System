...
class Controller_Reminder extends Admin_Controller 
{
    ...
    */
	public function upload_file()
    {
        ...
        $config['upload_path'] = 'assets/images/reminder_file';
        $config['file_name'] =  'RE-'."-".$this->input->post('title').'-'.uniqid();
    ...
    */
	...
	{      
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            // print_r($_FILES['agreement_file']);
            if($_FILES['agreement_file']['size'] > 0) {
                $upload_file = $this->upload_file();
                $upload_file = array('file' => $upload_file);
                print_r($upload_file);
                $this->model_reminder->updateFile($upload_file, $reminder_id);
            }
