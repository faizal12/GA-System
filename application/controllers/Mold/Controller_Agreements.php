...
class Controller_Agreements extends Admin_Controller 
{
    ...
    */
	...
	{
        if(!in_array('viewAgreement', $this->permission)) {
	...
	public function fetchAgreementData()
	{
		...
		foreach ($data as $key => $value) {
            ...
            if(in_array('updateAgreement', $this->permission)) {
    			$buttons .= '<a href="'.base_url('Controller_Agreements/update/'.$value['id']).'" class="btn btn-warning btn-xs"><i class="fa fa-edit"></i></a>';
            }
            ...
            $email = ($value['email'] == 1) ? '<span class="badge badge-success">Email</span>' : '<span class="badge badge-warning">Not Yet</span>';
